...
class JarvisLive:
    ...
    async def _listen_audio(self):
        ...
        # Queue used only for LOCAL wake-word detection.
        self._wake_audio_queue = asyncio.Queue()
        ...
        def callback(indata, frames, time_info, status):
            ...
            # Microphone remains available locally, but we don't
            # process it while JARVIS is speaking.
            if jarvis_speaking:
                return
        ...
        try:
            with sd.InputStream(
                ...
                while True:
                    ...
                    # Now send microphone audio to Gemini.
                    # =====================================================
                    ...
                    # =====================================================
                    # WAKE-WORD MODE
                    # Audio is processed locally by openWakeWord.
                    # It is NOT sent to Gemini.
                    # =====================================================
                    ...
                    while len(self._wake_buffer) >= FRAME_BYTES:
    
                        frame_bytes = bytes(
                            self._wake_buffer[:FRAME_BYTES]
                        )
    
                        del self._wake_buffer[:FRAME_BYTES]
                        frame = np.frombuffer(
                            frame_bytes,
                            dtype=np.int16
                        )
                        ...
                        if score >= self._wake_threshold:
                            ...
                            print(
                                f"[JARVIS] 🔓 Wake word detected "
                                f"(score={score:.2f})"
                            )
    
                            # Activate Gemini microphone.
                            ...
                            self.ui.write_log(
                                "SYS: Wake word detected — listening."
                            )
    
                            self.ui.set_state("LISTENING")

                            # IMPORTANT:
                            # Do NOT send the wake-word audio itself
                            # to Gemini.
                            break
            ...
            while True:
                try:
                    ...
                    if (
                        ...
                        # Return to wake-word mode after every response.
                        self._wake_active = False
                        self._wake_buffer.clear()
                        self._wake_model.reset()
